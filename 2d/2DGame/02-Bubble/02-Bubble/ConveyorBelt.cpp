...
void ConveyorBelt::render() {
	for (int i = 0; i < chunks.size(); ++i)
		chunks[i]->render();
}

void ConveyorBelt::init(int id, int size, const glm::ivec2 &tileMapPos, ShaderProgram &shaderProgram) {
	chunks.clear();
	chunks.resize(size);
...
void ConveyorBelt::setPosition(const glm::vec2 &pos)
{
	posObject = pos;
